...
const lockState = {
  ...
  step: 0,            // dial index 0-11, advances per keystroke (visual only)
  showPass: false,
  error: '',
  shaking: false,
  busy: false,
...
function renderLock(){
  ...
  const heading = isSetup
    ? { title:'ساخت رمز عبور اصلی', desc:'این رمز، GPsaver رو روی همین مرورگر قفل می‌کنه. اگه فراموشش کنی، راهی برای بازیابی وجود نداره — پس جایی یادداشتش کن.' }
    : { title:'GPsaver قفله', desc:'برای ورود، رمز عبور اصلی رو وارد کن.' };
    ...
    <div class="lock-stage" id="lockStage">
      ...
      <form class="lock-form" id="lockForm" autocomplete="off">
        <div class="lock-input-wrap">
          <input
            type="${lockState.showPass ? 'text' : 'password'}"
            class="lock-input lock-input--pin"
            id="lockPassInput"
            placeholder="${isSetup ? '۴ رقم بساز…' : 'رمز ۴ رقمی'}"
            autocomplete="new-password"
            inputmode="numeric"
            pattern="[0-9]*"
            maxlength="4"
          >
          <button type="button" class="lock-input-toggle" id="lockToggleVis" tabindex="-1">
            ${lockIcon(lockState.showPass ? 'eyeOff' : 'eye')}
          </button>
        </div>
        ...
        <button type="submit" class="lock-submit" id="lockSubmitBtn">
          ${lockIcon(isSetup ? 'check' : 'lock')}
          <span>${isSetup ? 'ساخت و ورود' : 'باز کردن GPsaver'}</span>
        </button>
...
function sanitizePinInput(el){
  ...
  const digitsOnly = el.value.replace(/[^0-9]/g, '').slice(0, 4);
...
function bindLockEvents(){
  const input = document.getElementById('lockPassInput');
  const confirmInput = document.getElementById('lockPassConfirm');
  const form = document.getElementById('lockForm');
  const toggleVis = document.getElementById('lockToggleVis');
  const forgotBtn = document.getElementById('lockForgot');

  if(input){
    input.addEventListener('input', () => {
      sanitizePinInput(input);
      advanceDial(input.value.length);
    });
  }
  if(confirmInput){
    confirmInput.addEventListener('input', () => {
      sanitizePinInput(confirmInput);
    });
  }
  if(toggleVis){
    toggleVis.addEventListener('click', () => {
      lockState.showPass = !lockState.showPass;
      const wasEnter = lockState.mode === 'enter';
      const val1 = document.getElementById('lockPassInput').value;
      const val2 = document.getElementById('lockPassConfirm') ? document.getElementById('lockPassConfirm').value : null;
      renderLock();
      document.getElementById('lockPassInput').value = val1;
      if(val2 !== null && document.getElementById('lockPassConfirm')) document.getElementById('lockPassConfirm').value = val2;
    });
  }
  if(forgotBtn){
  ...
  if(form){
    form.addEventListener('submit', handleLockSubmit);
  }
}
...
async function handleLockSubmit(e){
  ...
  if(lockState.mode === 'setup'){
    ...
    if(pass !== confirmVal){
      shakeLock('دو رمز باهم یکی نیستن');
      return;
    }
    lockState.busy = true;
...
function renderRail(){
  ...
  <aside class="rail">
    ...
    <div class="rail__foot">
      <strong>ذخیره‌سازی محلی:</strong> همه‌چیز فقط توی همین مرورگر می‌مونه. هیچ اطلاعاتی جایی ارسال نمیشه.
    </div>
  </aside>`;
}
...
function handleClick(e){
  ...
  switch(action){
    ...
    case 'save-entry': {
      ...
      if(state.modal.editingId){
        const existing = c.entries.find(x => x.id === state.modal.editingId);
        if(existing){
    ...
    case 'delete-entry': {
      const c = getActiveCompartment();
      const entry = c.entries.find(x => x.id === t.dataset.id);
      state.modal = { type:'confirm-delete-entry', entry };
      render();
      break;
    }
