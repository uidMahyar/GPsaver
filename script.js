...
function renderLock(){
  ...
  root.innerHTML = `
    <svg class="filter-defs-svg" aria-hidden="true">
      <filter id="keypadElectric" x="-50%" y="-50%" width="200%" height="200%">
        <feTurbulence type="turbulence" baseFrequency="0.09" numOctaves="6" result="noise" seed="1">
          <animate attributeName="seed" from="1" to="30" dur="4s" repeatCount="indefinite" />
        </feTurbulence>
...
function renderKeypad(){
  ...
  const cells = keys.map(k => {
    ...
    if(k === 'del'){
      return `<button type="button" class="keypad__key keypad__key--del" data-key="del" aria-label="حذف رقم آخر">
        <span class="keypad__glow"></span>
        <span class="keypad__border"></span>
        <span class="keypad__content">${lockIcon('x')}</span>
      </button>`;
    }
    return `<button type="button" class="keypad__key" data-key="${k}">
      <span class="keypad__glow"></span>
      <span class="keypad__border"></span>
      <span class="keypad__content">${k}</span>
    </button>`;
  }).join('');
